...
export const detailConfigs:Record<string,DetailConfig>={
  ...
  health:{...standard('健康档案管理'),titleField:'subjectName',subtitle:d=>`档案编号：${d.archiveId}｜健康主体：${d.subjectId}｜${d.userName} / ${d.relation}`,statusFields:['aiReadStatus','archiveStatus'],tabs:['档案概览','基础病与过敏史','病历记录','检查报告','诊断记录','用药记录','AI可读取范围','数据审计'],actions:[{label:'查看病历',route:'records'},{label:'查看报告',route:'reports',tone:'primary'},{label:'查看入档记录',route:'archive'}]},
  records:{...standard('病历管理'),subtitle:d=>`病历编号：${d.id}｜用户：${d.userName} ${d.userId}｜健康主体：${d.subjectName} ${d.subjectId}`,statusFields:['ocrStatus','structuredStatus','aiSearchStatus','reviewStatus','archiveStatus'],tabs:['病历原文','结构化字段','AI可检索状态','关联报告','关联诊断','操作日志'],actions:[{label:'查看关联报告',route:'reports'},{label:'提交复核',tone:'primary'}]},
  reports:{...standard('检查报告管理'),subtitle:d=>`报告编号：${d.id}｜用户：${d.userName} ${d.userId}｜健康主体：${d.subjectName} ${d.subjectId}`,statusFields:['ocrStatus','structuredStatus','aiStatus','reviewStatus','archiveStatus'],tabs:['报告概览','报告原图 / OCR文本','结构化字段','AI解读','医学复核','关联问诊','操作日志'],actions:[{label:'查看完整解读任务',route:'interpretation',tone:'primary'},{label:'查看健康档案',route:'health'}]},
  diagnoses:{...standard('诊断记录管理',['诊断概览','来源病历','关联报告','状态变更','操作日志']),titleField:'diagnosisName',subtitle:d=>`诊断编号：${d.id}｜用户：${d.userName} ${d.userId}｜健康主体：${d.subjectName} ${d.subjectId}`,statusFields:['diagnosisConclusion','confirmationStatus','currentStatus','archiveStatus']},
  symptoms:{...standard('症状记录管理',['症状概览','来源对话','入档记录','风险研判','操作日志']),titleField:'symptomName',subtitle:d=>`症状编号：${d.id}｜用户：${d.userName} ${d.userId}｜健康主体：${d.subjectName} ${d.subjectId}`,statusFields:['severity','redFlag','archiveStatus']},
  consults:{...standard('AI问诊管理'),subtitle:d=>`会话编号：${d.id}｜${d.consultType==='AI诊室'?'AI诊室 · 结构化追问':`${d.consultType} · ${d.consultMode}`}｜用户：${d.userName} ${d.userId}`,statusFields:['consultType','consultStatus','riskLevel','archiveStatus'],tabs:['会话概览','多轮问诊记录','症状结构化','多模态附件','图像分析结果','隐私合规','风险规则命中','AI结论与导诊','模型与工具调用','数据入档与审计'],actions:[{label:'查看症状记录',route:'symptoms'},{label:'查看入档记录',route:'archive',tone:'primary'},{label:'查看命中规则',route:'rules'}]},
  ...
  agents:{...standard('医生智能体管理'),subtitle:d=>`智能体编号：${d.id}｜${d.agentType}｜${d.doctor} · ${d.department}`,statusFields:['authStatus','reviewStatus','publishStatus','testResult'],tabs:['基础配置','专科边界','知识库引用','工具权限','测试记录','用户反馈','上线审核 / 版本记录','运行监控','操作日志'],actions:[{label:'查看绑定医生',route:'doctors'},{label:'查看Prompt版本',route:'prompts',tone:'primary'}]},
