...
export const formSchemas:Record<string,Field[]>={
  ...
  symptoms:[req('userId','用户ID'),req('subjectId','健康主体ID'),req('subjectName','主体姓名'),req('sourceType','来源类型','select',['AI诊室','用户手动记录','医生病历提取','报告解读提取','后台管理员录入']),f('sourceSessionId','来源会话ID'),req('chiefComplaint','主诉','textarea'),req('name','症状名称'),f('bodyPart','症状部位'),f('nature','症状性质'),f('duration','持续时间'),req('severity','严重程度','select',['轻','中','重','急症']),f('companions','伴随症状'),req('redFlag','是否红旗症状','select',['是','否','待排查']),f('completeness','信息完整度'),f('diagnosisId','关联诊断ID'),req('archiveStatus','是否入档','select',['未入档','待入档','已入档','待人工复核'])],
  consults:[req('userId','用户ID'),req('userName','用户姓名'),req('subjectId','健康主体ID'),req('subjectName','主体姓名'),req('relation','成员关系','select',['本人','父亲','母亲','配偶','儿子','女儿']),req('chiefComplaint','主诉','textarea'),f('currentSymptoms','当前症状'),f('progress','问诊进度'),f('rounds','追问轮次','number'),req('consultStatus','问诊状态','select',['问诊中','待用户补充','信息已足够','已生成结论','已中断','已转人工','已触发风险']),req('riskLevel','风险等级','select',['低风险','中风险','高风险','紧急风险']),f('rule','命中规则'),f('department','推荐科室'),req('modelName','模型名称','select',['deepseek-chat','MedGPT-4.1']),req('promptVersion','Prompt版本'),req('archiveStatus','入档状态','select',['未入档','待入档','已入档','待人工复核'])],
