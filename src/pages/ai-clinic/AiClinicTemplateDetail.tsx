...
import { aiClinicSessions, aiClinicTemplates, globalSlotDefinitions, templateSlotBindings } from '../../mock/aiClinicAdminData'
...
export default function AiClinicTemplateDetail({ templateId, onNavigate }: { templateId: string; onNavigate: (key: string) => void }) {
  ...
  const slots = templateSlotBindings.filter(item => item.templateId === template.id && item.templateVersion === template.version)
  const totalWeight = slots.filter(slot => slot.enabled === '已启用').reduce((sum, slot) => sum + slot.weight, 0)
  const coreWeight = slots.filter(slot => slot.enabled === '已启用' && slot.core === '是').reduce((sum, slot) => sum + slot.weight, 0)
  const enabledSlotCount = slots.filter(slot => slot.enabled === '已启用').length
  const standardSlotLabel = (globalSlotId: string) => {
    const globalSlot = globalSlotDefinitions.find(item => item.id === globalSlotId)
    return globalSlot ? `${globalSlot.id} · ${globalSlot.name} · ${globalSlot.code}` : `${globalSlotId} · 未匹配标准槽位`
  }
  return <div>
    ...
    <section className="session-hero template-hero">
      ...
      <div className="session-status">
        <StatusTag value={template.publishStatus} />
        <StatusTag value={template.status} />
        <b>{template.evaluationOutputThreshold}</b><span>评估输出阈值</span>
        <b>{template.outputStrategy}</b><span>输出策略</span>
      </div>
    </section>
    ...
    <section className="session-tab-panel">
      {tab === '基础配置' && <Info items={[
        ...
        ['当前版本', template.version], ['模板优先级', String(template.priority)], ['槽位数量', `${enabledSlotCount || template.slotCount}个`], ['核心槽位数量', `${slots.filter(slot => slot.core !== '否').length || template.coreSlotCount}个`],
        ['评估输出阈值', template.evaluationOutputThreshold], ['输出策略', template.outputStrategy], ['自动结论阈值', template.autoConclusionScore], ['直接结论最低分', template.directConclusionMinScore], ['最大默认问诊轮次', `${template.maxRounds}轮`],
      ...
      {tab === '模板槽位' && <>
        <div className="business-info-grid session-info-grid">
          {[
            ['模板总权重', `${totalWeight}分`],
            ['核心槽位权重', `${coreWeight}分`],
            ['评估输出阈值', template.evaluationOutputThreshold],
            ['必须完成危险症状筛查', template.riskScreeningRequired],
            ['风险筛查未完成时进度上限', '80%'],
            ['已启用槽位数', `${enabledSlotCount}个`]
          ].map(item => <div key={item[0]}><span>{item[0]}</span><strong>{item[1]}</strong></div>)}
        </div>
        <Table columns={['顺序', '模板槽位编号', '标准槽位', '槽位名称', '展示组件', '是否必填', '是否核心', '权重', '否定回答规则', '不清楚规则', '有效完成最低系数', '是否需要确认', '最大追问次数', '状态', '操作']} rows={slots.map(slot => [
          slot.order,
          slot.id,
          standardSlotLabel(slot.globalSlotId),
          slot.displayName,
          slot.component,
          slot.required,
          slot.core,
          `${slot.weight}分`,
          slot.negativeAnswerRule,
          slot.unclearRule,
          slot.minimumCompletionCoefficient,
          slot.needConfirmation,
          `${slot.maxAskCount}次`,
          slot.enabled,
          '查看规则'
        ])} />
      ...
      {tab === '直接结论规则' && <Info items={template.id === 'TMP000' ? [
        ['评估输出阈值', template.evaluationOutputThreshold],
        ['输出策略', template.outputStrategy],
        ['允许输出内容', '症状摘要、当前风险等级、推荐科室、建议就医时机、需要警惕的危险表现'],
        ['禁止输出内容', '明确疾病诊断'],
        ['风险筛查要求', template.riskScreeningRequired]
      ] : [['最低分', template.directConclusionMinScore], ['自动结论阈值', template.autoConclusionScore], ['评估输出阈值', template.evaluationOutputThreshold], ['输出策略', template.outputStrategy], ['风险筛查要求', template.riskScreeningRequired], ['高风险时', '不生成普通结论，优先安全提示']]}/>}
...
function Table({ columns, rows }: { columns: string[]; rows: Array<Array<ReactNode>> }) {
  return <div className="business-table session-table"><table><thead><tr>{columns.map(col => <th key={col}>{col}</th>)}</tr></thead><tbody>{rows.map((row, i) => <tr key={i}>{row.map((cell, j) => <td key={j}>{typeof cell === 'string' && /启用|停用|草稿/.test(cell) ? <StatusTag value={cell} /> : cell}</td>)}</tr>)}</tbody></table></div>
}
